...
別のバージョンのcurlがインストールされている場合、ライブラリ参照のエラーが出ることがある。
→libcurl4パッケージをアンインストールする。
...
HTTP3が有効になっている場合は、--http3オプションが使用可能。<br />
